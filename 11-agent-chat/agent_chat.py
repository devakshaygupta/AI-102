...
async def main():
    ...
    async with (
        ...
        incident_manager_agent = AzureAIAgent(
            client=client,
        ...
        # Create a Semantic Kernel agent for the Azure AI devops manager agent
        devops_manager_agent = AzureAIAgent(
            client=client,
        ...
        chat = AgentGroupChat(
            agents=[incident_manager_agent, devops_manager_agent],
            termination_strategy=ApprovalTerminationStrategy(
                agents=[incident_manager_agent],
                maximum_iterations=10,
                automatic_reset=True
            ),
            selection_strategy=SelectionStrategy(agents=[incident_manager_agent, devops_manager_agent]),
        )
