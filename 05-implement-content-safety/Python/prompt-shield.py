    ...
    def make_decision(
        ...
        for category, threshold in reject_thresholds.items():
            if threshold not in (-1, 0, 2, 4, 6):
                raise ValueError(
                    "RejectThreshold can only be in (-1, 0, 2, 4, 6)")
            ...
            if cate_detect_res is None or "severity" not in cate_detect_res:
                raise ValueError(
                    f"Can not find detection result for {category}")
...
if __name__ == "__main__":
    ...
    subscription_key = os.getenv("CONTENT_SAFETY_SUBSCRIPTION_KEY")
    ...
    decision_result = content_safety.make_decision(
        detection_result, reject_thresholds)
